...
def fetch_reformat_data(formatted_mutations, date_range, location=None):
    mutation_type = (
        "aminoAcid"  # as we care about amino acid mutations, as in resistance mutations
    )
    all_data = asyncio.run(
        wiseLoculus.fetch_mutation_counts(
            formatted_mutations, mutation_type, date_range, location
        )
    )
...
def plot_resistance_mutations(df):
    ...
    df_processed = (
        df.replace({None: np.nan, ",": ""}, regex=True)
    ...
    height = max(
        400, len(df_processed.index) * 20 + 100
    )  # Base height + per mutation + padding for title/axes
    ...
    margin_l = max(
        80, max_len_mutation_label * 7 + 30
    )  # Min margin or calculated, adjust multiplier as needed
    ...
    if num_cols > 0:
        ...
        if (
            num_cols > 2 and num_cols // 2 != num_cols - 1
        ):  # Avoid duplicate if middle is last
...
def app():
    ...
    st.write(
        "This page allows you to visualize the numer of observed resistance mutations over time."
    )
    st.write(
        "The data is fetched from the WISE-CovSpectrum API and currently cointains demo data for Feb-Mar 2025."
    )
    st.write(
        "The sets of resistance mutations are provide from Stanfords Coronavirus Antivirial & Reistance Database. Last updated 05/14/2024"
    )
    st.write(
        "This is a demo frontend to later make the first queries to SILO for wastewater data."
    )
    st.markdown("---")
    ...
    st.write(
        "Note that mutation sets `3CLpro` and `RdRP`refer to mature proteins, "
    ...
    date_range = st.date_input(
        "Select a date range:",
        [pd.to_datetime("2025-02-10"), pd.to_datetime("2025-03-08")],
    )
    ...
    if not mutaton_counts_df.empty:
        if mutaton_counts_df.isnull().all().all():
            st.error(
                "The fetched data contains only NaN values. Please try a different date range or mutation set."
            )
        else:
    ...
    st.write(
        "In the long term, GenSpectrum will provide a dashboard to visualize the mutations over time."
    )
    st.write(
        "Yet currently, the below component only shows mutations above an unknown threshold."
    )
    st.write(
        "This is under investigation and will be addresed by the GenSpectrum team."
    )
